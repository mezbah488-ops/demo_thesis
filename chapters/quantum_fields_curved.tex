...
\label{ch:qft-curved}
We now develop the general formalism of quantum field theory in curved
spacetime. The central lesson is that the notion of a particle, and
hence of a vacuum state, is not an intrinsic property of a quantum
field but depends on the choice of mode decomposition, which in turn
depends on the observer and the geometry. This is a fundamental
departure from flat spacetime, where Poincar\'{e} invariance singles
out a unique vacuum. The Bogoliubov transformation formalism developed
here is the technical tool that quantifies this observer-dependence,
and it will be applied directly in the following chapters to compute
particle production in specific spacetimes. It also provides the
conceptual foundation for understanding why the effective potential,
which we computed in Parts~\ref{part:eff-action} and
\ref{part:finite-temp} assuming a unique vacuum, requires
generalization in curved spacetime.
...
\section{Ambiguity in the Notion of Particles and Bogoliubov
  Transformations}
In Minkowski spacetime the construction of a vacuum state and Fock
space is unambiguous because there is a natural set of modes,
the plane waves \eqref{eq:plane-waves}, singled out by the Poincar\'{e}
symmetry of the background. The vector $\partial/\partial t$ is a
Killing vector of Minkowski space, orthogonal to the spacelike
hypersurfaces $t = \text{const}$, and the plane wave modes are its
eigenfunctions with eigenvalues $-i\omega$ for $\omega > 0$. The
vacuum is then the unique state invariant under the Poincar\'{e} group.

In curved spacetime the Poincar\'{e} group is no longer a symmetry,
and in general there are no Killing vectors at all with which to define
positive-frequency modes. In special cases, such as de Sitter space or
the Schwarzschild exterior, there may be Killing vectors associated
with particular symmetries, and one can define natural coordinates
adapted to them. But even when such coordinates exist, they do not
exhibit the same privileged status as the inertial coordinates of
Minkowski space. The spirit of general covariance is that no coordinate
system is physically preferred, and so no mode decomposition is
intrinsically more natural than any other. The ambiguity in the choice
of modes is therefore not a technical inconvenience but a fundamental
feature of quantum field theory in curved spacetime.
